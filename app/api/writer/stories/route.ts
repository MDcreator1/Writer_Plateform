...
const storySchema = z.object({
  ...
  if (value.synopsis.length < 20) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Summary should be at least 20 characters.", path: ["synopsis"] });
  }
  if (value.tags.length < 2) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Please add at least 2 tags.", path: ["tags"] });
  }
});
...
async function getUniqueSlug(title: string, currentStoryId?: string) {
  ...
  while (true) {
    ...
    if (!existing || existing.id === currentStoryId) return candidate;
...
export async function GET() {
  try {
    ...
    return fail("Unable to fetch stories", 500);
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const existingDraft = body.draftId ? await prisma.story.findUnique({ where: { id: body.draftId } }) : null;
    ...
    const authorName = user.displayName || user.username || user.email || "Writer";
    const storyData = {
    ...
    if (body.projectId) {
      const existingLink = await prisma.studioProjectLink.findUnique({ where: { projectId: body.projectId } });
      if (!existingLink) {
        await prisma.studioProjectLink.create({
          data: { projectId: body.projectId, projectTitle: story.title, storyId: story.id, source: "STUDIO" }
        });
      } else if (existingLink.storyId !== story.id) {
        await prisma.studioProjectLink.update({ where: { projectId: body.projectId }, data: { storyId: story.id } });
      }
    }
