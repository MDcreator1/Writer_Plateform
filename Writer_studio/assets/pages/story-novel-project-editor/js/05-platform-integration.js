const STUDIO_PLATFORM_URL_KEY = 'lm_writer_platform_url';
const STUDIO_PLATFORM_KEY_KEY = 'lm_writer_platform_key';
...
function studioPlatformConfig() {
  const defaultUrl = window.location.port === '5500'
    ? 'http://localhost:3000'
    : window.location.protocol.startsWith('http') ? window.location.origin : 'http://localhost:3000';
  return {
    apiUrl: (localStorage.getItem(STUDIO_PLATFORM_URL_KEY) || defaultUrl).replace(/\/+$/, ''),
    key: localStorage.getItem(STUDIO_PLATFORM_KEY_KEY) || ''
  };
}

function saveStudioPlatformConfig() {
  const apiUrl = document.getElementById('studioPlatformApiUrl')?.value.trim().replace(/\/+$/, '') || '';
  const key = document.getElementById('studioPlatformKey')?.value.trim() || '';
  if (apiUrl) localStorage.setItem(STUDIO_PLATFORM_URL_KEY, apiUrl);
  if (key) localStorage.setItem(STUDIO_PLATFORM_KEY_KEY, key);
  else localStorage.removeItem(STUDIO_PLATFORM_KEY_KEY);
  studioPlatformSetMessage('Connection settings saved.');
}
    ...
    <div class="studio-platform-modal" id="studioPlatformModal" aria-hidden="true">
      <section class="studio-platform-panel" role="dialog" aria-modal="true" aria-labelledby="studioPlatformTitle">
        ...
        <div class="studio-platform-scroll">
          <section class="studio-platform-settings">
            <label><span>Platform URL</span><input id="studioPlatformApiUrl" type="url" value="${studioPlatformEscape(config.apiUrl)}" placeholder="http://localhost:3000"></label>
            <label><span>Integration key</span><input id="studioPlatformKey" type="password" value="${studioPlatformEscape(config.key)}" autocomplete="off"></label>
            <button type="button" data-studio-platform-action="save-settings">Save connection</button>
          </section>
...
async function handleStudioPlatformAction(event) {
  ...
  if (action === 'save-settings') saveStudioPlatformConfig();
...
function initStudioPlatformIntegration() {
  ...
  if (studioPlatformLaunchTarget?.platformUrl) {
    localStorage.setItem(STUDIO_PLATFORM_URL_KEY, studioPlatformLaunchTarget.platformUrl.replace(/\/+$/, ''));
  }
  ...
  button.title = 'Writer Platform Cloud Stories';
  button.addEventListener('click', openStudioPlatformIntegration);
