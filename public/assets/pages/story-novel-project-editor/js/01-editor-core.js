...
function standaloneFocusEditorTargetUrl() {
  const targetUrl = new URL('/admin/studio', window.location.href);
...
async function init() {
  ...
  if (!hasActiveStory()) {
    ...
    const launchTarget = typeof readStudioPlatformLaunchTarget === 'function' ? readStudioPlatformLaunchTarget() : null;
    if (launchTarget) {
      return;
    }
    hideAppLoader(true);
